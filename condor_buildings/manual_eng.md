...
- **Built only once** — the tiles leave each other notes in a `border_pylons.json` file next to the landscape's map data, keyed by the pylon's OSM node id. Whoever builds second reads the note, does not build the finished pylon again, attaches its own cables to the recorded points, and does not draw a span the first tile already drew in full.

  Written for every pylon:
