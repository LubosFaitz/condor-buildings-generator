...
Anything carrying the word `building` in OSM used to be built, whatever stood after it. That put houses where none exist — a coastal staircase, for instance, which the mapper had explicitly marked as not a building.

The value is now checked as well. Nothing is built for:

| OSM tagging | what it is |
|---|---|
| `building = no` | the mapper states outright it is not a building |
| `building = ruins` | a ruin |
| `building = roof` | a roof on posts, no walls - bus shelter, fuel station canopy, carport |
| `building = bridge` | a bridge |
| `building = construction` | still being built, nothing stands there yet |
| `amenity = shelter` | a shelter with a bench, even when tagged as a building |

Everything else is built as before - houses, apartments, halls, churches, garages. It applies to MSprint buildings too.
...
### Pylons on the tile border
Where a line runs out of the tile, the plugin builds **one pylon beyond the edge** so the cables have somewhere to go.

- **Height taken from the neighbouring terrain** — such a pylon is placed on the **real ground of the neighbouring tile** (its height map is read), not on the edge of the tile being built. Both tiles therefore put the same pylon at the same height and the cables have no kink at the seam.
- **Built only once** — the tiles leave each other notes in a `border_pylons.json` file next to the landscape's map data. Every pylon recorded there carries its position, height, orientation and **the points the cables attach to** (the crossarm tips). Whoever builds second reads the note, does not build the finished pylon again, attaches its own cables to the recorded points, and does not draw a span the first tile already drew in full.
- **When to delete the file** — the notes are always used while they are there. If you download the map data again and a line genuinely moves, delete `border_pylons.json` and the plugin recreates it from the current map on the next run. Otherwise there is no need to touch it. When the file is missing, corrupted or cannot be written, generation behaves exactly as before.

### Pylon spacing
Around every pylon origin there is a **keep-out circle** whose diameter is the crossarm span plus a margin, and the circles of two pylons must never overlap — so the cables of two parallel lines cannot cross. The minimum centre-to-centre distance of a pair is the sum of their radii.
...
Two large pylons therefore need 28.7 m, a large and a medium one 20.85 m, two small ones 2 m. The check also measures the **distance between the line routes themselves**, not just origin against origin — two lines whose pylons are staggered rather than opposite each other are still kept apart where their routes come closest.

Substations, entry pylons and gantries have their own spacing and are not affected by this.
...
**Where the data comes from.** Ways and nodes alike. These are all the tags the plugin uses:
...
| tree_lined on a road or a waterway | the trees lining it — also tree_lined:left, tree_lined:right, tree_lined:both, denotation=avenue and the deprecated alley=left/right/both |
| natural=tree | a single tree |

The values "no" and "separate" are deliberately not built — with "separate" the trees are
drawn as their own way next to it, so they would otherwise come out twice.
...
**The post** is the ready-made model `assets/pylons/fence_post.obj` — a triangular wooden
...
**One fence line = one object** `fence`. **Merge** joins a patch's fences together and the
result is named **`pylones`** — the same object the powerlines and the aerialways use,
since they share the material and the texture. When the patch already has `pylones`, the
fences are joined straight into it; when it has none (no powerlines, no aerialways), the
merged fence carries that name anyway.

File mode does the same thing while writing the OBJ. The exporter always puts `pylones`
last in the file, so the fence is simply appended to it and becomes part of it; in a patch
without powerlines a block `o pylones` is opened for the fence instead.

**Fences are never built into LOD1.** A fence is a small detail that is only seen from
close up, so there is nothing worth putting into the distant LOD. With **LOD1** alone
selected in *Output*, pressing *Import* only reports `Barrier fences are not created in
LOD1` and stops — nothing is downloaded and nothing is computed. With **LOD0** or **Both
LODs** the fence is built into LOD0 only. File mode behaves the same way: the fences are
written into `o<patch>.obj` and `o<patch>_LOD1.obj` is skipped.
