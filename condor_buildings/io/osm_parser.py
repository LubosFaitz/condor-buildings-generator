...
# ``building`` tag values that are NOT a building - these are never generated:
#   'no'           - the mapper says explicitly it is not a building (ECP10 stairs, way 75656944)
#   'ruins'        - a ruin (Birnbeck Pier and Burnbeck Pier - piers, not houses)
#   'roof'         - a shelter with no walls, 3-7 m across in the sample data
#   'bridge'       - a bridge, not a building
#   'construction' - under construction, nothing standing there yet
NOT_A_BUILDING = {'no', 'ruins', 'roof', 'bridge', 'construction'}

# Objects tagged building=yes where another tag makes it clear it is not a house.
#   amenity=shelter - a shelter with a bench; way 916027894 tags the whole 213 m long
#                     Grand Pier in Weston-super-Mare like this
NOT_A_BUILDING_AMENITY = {'shelter'}
...
def parse_osm_file(
    ...
    for rel_id, relation in relations.items():
        ...
        # Check if it's a building - the VALUE matters, not just the key
        if relation.tags.get('building', 'no') in NOT_A_BUILDING:
            continue
        if relation.tags.get('amenity') in NOT_A_BUILDING_AMENITY:
            continue
    ...
    for way_id, way in ways.items():
        ...
        # Check if it's a building - the VALUE matters, not just the key
        if way.tags.get('building', 'no') in NOT_A_BUILDING:
            continue
        if way.tags.get('amenity') in NOT_A_BUILDING_AMENITY:
            continue
