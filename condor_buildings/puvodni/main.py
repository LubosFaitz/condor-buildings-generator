...
def _generate_wind_turbines_group(osm_path, projector, terrain):
    ...
    meshes, count = generate_wind_turbines_mesh(parse_result.turbines, terrain)
    if not meshes:
        return None, None, 0
    ...
    for i, mesh in enumerate(meshes):
        ...
        groups0[key] = mesh
        groups1[key] = copy.deepcopy(mesh)
...
def run_pipeline(
    ...
    if config.generate_powerlines:
        try:
            wt0, wt1, wt_count = _generate_wind_turbines_group(osm_path, projector, terrain)
            if wt0 is not None:
