...
What it does (the whole core):
  ...
  One post is a triangular prism, standing 1.30 m above the terrain and
...
# After Merge (and in file mode) the fence becomes part of the powerline object, so it
# carries that name - also in a patch that has no powerlines and no aerialways at all.
MERGED_NAME = "pylones"
...
# --- the post ---------------------------------------------------------------
# The post is NOT generated any more - it is the fence_post.obj model, a triangular
# wooden post whose origin sits at ground level: 1.00 m of it is below the
...
class CONDOR_OT_import_fences(Operator):
    ...
    bl_description = ("Generate fences from OSM (barrier=fence) as 'fence' objects - a "
                      "triangular post every few metres plus two wires")
    ...
    def execute(self, context):
        ...
        # LOD1 gets NO fences at all - a fence is a small, close-up detail, so there is
        # nothing worth putting into the distant LOD. With LOD1 alone selected the whole
        # thing stops right here, BEFORE anything is downloaded or any terrain is read.
        if props.output_lod == 'LOD1':
            self.report({'INFO'}, "Barrier fences are not created in LOD1")
            return {'FINISHED'}
        ...
        for patch_id in patch_ids:
            ...
            # collection differs.
            # LOD0 only - the fence is never built into LOD1 (see the check in execute)
            lod_variants = [""]
...
class CONDOR_OT_merge_fences(Operator):
    ...
    bl_description = ("Join the fences of each patch into the 'pylones' object - together "
                      "with the powerlines and aerialways when the patch has them, and "
                      "under that name even when it has none. Your own edits are kept")
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        for (col_name, _lod), objs in groups.items():
            ...
            # The fence shares the powerline material, so it belongs in the very same
            # object: when this patch already has 'pylones' (powerlines / aerialways), the
            # fences are joined INTO it and it stays the active one, so the result keeps
            # that name. Without it the merged fence is simply named 'pylones' as well.
            col = bpy.data.collections.get(col_name)
            pyl = next((o for o in (col.objects if col else [])
                        if o.type == 'MESH'
                        and o.name.split('.')[0] == MERGED_NAME
                        and o not in objs), None)
            target = pyl if pyl is not None else objs[0]
            members = ([pyl] + objs) if pyl is not None else objs
            if len(members) > 1:
                try:
        ...
        self.report({'INFO'}, f"Fences: merged into {merged} '{MERGED_NAME}' object(s), "
                              f"material '{MAT_NAME}'.")
        return {'FINISHED'}
...
def _append_fence_objects(obj_path, parts):
    """Append the fences to a file-mode patch OBJ as part of the 'pylones' object, keeping
    the SMOOTH per-vertex normals the geometry was built with.

    The fence shares the powerline material, so it belongs in the very same object. The
    exporter always writes 'pylones' LAST (see _condor_group_order in io/obj_exporter.py),
    so when the last object in the file is 'pylones', the geometry is simply appended with
    NO new 'o' line and becomes part of it. When there is no 'pylones' - no powerlines and
    no aerialways in this patch - the block is opened as 'o pylones' itself, so the fence
    ends up in an object of that name either way.

    The file is read and written ONCE, however many fences there are. True when something
    was written."""
    ...
    # is 'pylones' the object the file currently ends with?
    last_o = next((l.strip() for l in reversed(lines) if l.startswith("o ")), "")
    into_pylones = (last_o == f"o {MERGED_NAME}")
    ...
    for (verts, faces, uvs, norms) in parts:
        ...
        f_lines = ["f " + " ".join(
            ...
            for tri in tris]
        # the header is written once, and only when we are NOT continuing 'pylones'
        head = [] if (into_pylones or out) else ["", f"o {MERGED_NAME}", f"usemtl {MAT_NAME}"]
        out += head + v_lines + vt_lines + vn_lines + f_lines
        base_v += len(v_lines)
...
def _append_fences_after_export(obj_filepath):
    ...
    OBJ. ONLY o<patch>.obj (LOD0) - o<patch>_LOD1.obj gets no fences at all, and is left
    before anything is downloaded or any terrain is read."""
    import re
    if not getattr(bpy.context.scene, "condor_fence_batch", False):
        return
    m = re.match(r'^o(\d{6})\.obj$', os.path.basename(obj_filepath))
    if not m:
        return                      # not a patch OBJ, or it is the LOD1 one
    patch_id = m.group(1)
