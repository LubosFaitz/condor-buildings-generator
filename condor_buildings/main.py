...
def _generate_powerline_group(osm_path, projector, terrain, draw_balls=False,
    ...
    Border towers (the first node just beyond the patch) take their foot height
    from the ADJACENT patch's terrain (NeighborTerrain), so they line up with that
    patch. They are also written into a shared log (``border_pylons.json``, next to
    the map data), so the neighbouring patch does not build a second, differently
    turned copy of the same tower but only strings its cables onto it.
    ...
    # Real ground for the towers standing just over the patch border (loaded from
    # the neighbouring patch on demand); None when the patch info is missing.
    neighbor = None
    if heightmaps_dir and patch_id is not None:
        from .generators.aerialway import NeighborTerrain
        neighbor = NeighborTerrain(heightmaps_dir, patch_id, translate_x, translate_y)

    # Shared note-book next to the map data: a tower standing just over the border is
    # built by ONE patch only, whoever comes second reads its position/height/yaw from
    # here and just hangs its cables on it. Without the patch frame there is nothing to
    # convert the notes with, so the log is skipped (every patch builds its own copy).
    border_log = None
    if patch_id is not None and translate_x is not None and translate_y is not None:
        from .io.border_pylons import BorderPylonLog, FILE_NAME as BORDER_PYLON_FILE
        border_log = BorderPylonLog(
            os.path.join(os.path.dirname(osm_path), BORDER_PYLON_FILE),
            patch_id, translate_x, translate_y,
        )
    ...
    mesh_lod0, mesh_lod1, pl_stats = generate_powerline_meshes(
        ...
        substation_polygons=sub_polys, neighbor=neighbor,
        border_log=border_log, patch_id=patch_id,
    )
...
def _read_substation_polygons(osm_path, projector, min_size_m=100.0):
    ...
    Kept when the yard is a real switchyard (>= min_size_m across) OR when a power line
    actually runs into it - a traction substation or a distribution one is small but is
    still where the line ends, so it gets a gantry like any other. Only the little kiosks
    nothing is wired to are skipped. Pure ElementTree, so it works with or without
    Blender. Never raises.
    """
    ...
    # Every node any power line passes through, in patch coords - used to tell a
    # substation a line ends in from a kiosk nothing is connected to.
    line_pts = []
    for way in root.findall("way"):
        ...
        for nd in way.findall("nd"):
            ref = nd.get("ref")
            if ref in coords:
                line_pts.append(projector.project(*coords[ref]))
    ...
    for way in root.findall("way"):
        ...
        if max(max(xs) - min(xs), max(ys) - min(ys)) >= min_size_m:
            rings.append(ring)
            continue
        # Small yard: keep it only when a power line really runs into it.
        x0, x1 = min(xs), max(xs)
        y0, y1 = min(ys), max(ys)
        for (px, py) in line_pts:
            if x0 <= px <= x1 and y0 <= py <= y1 and _inside(px, py, ring):
                rings.append(ring)
                break
    return rings
