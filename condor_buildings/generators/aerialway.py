...
class NeighborTerrain:
    ...
    def _neighbor_id(self, x, y, bb):
        # Column numbers grow WESTWARD: local +X points east, but TranslateX
        # grows with the column index, so a HIGHER column number is a patch
        # further WEST. A point past the EAST edge therefore belongs to column
        # ``col - 1`` (and past the WEST edge to ``col + 1``) - hence the minus
        # sign below. Rows are the other way round: a higher row number is
        # further north, the same direction as local +Y, so ``row + drow``.
    ...
    def foot_z(self, x, y, current_terrain):
        ...
        nx = x + (ntx - self.tx)
        ny = y + (nty - self.ty)
        # Safety net: the shifted point must really land inside the neighbour's
        # own bbox. If it does not, this is not the right patch - return None so
        # the caller keeps its own fallback instead of using a height taken from
        # a completely different place in the landscape.
        nbb = terr.bbox
        if not (nbb.min_x <= nx <= nbb.max_x and nbb.min_y <= ny <= nbb.max_y):
            return None
