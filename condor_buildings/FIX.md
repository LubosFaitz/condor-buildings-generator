# Změny oproti Original/ — přesná místa vložení

---

## processing/mesh_grouper.py

**Za řádek 21** (za posledním importem) — vložit dvě nové funkce:
```python
...
**Řádek 48** — změnit signaturu `__init__`:
```diff
...
**Za řádek 57** (za docstringem `__init__`) — přidat `self.is_lod0`:
```diff
+        self.is_lod0 = is_lod0
```

**Za řádek 61** (za `self.houses = MeshData()`) — přidat `self.gabled_roofs`:
```diff
+        self.gabled_roofs = MeshData()
```

**Za řádek 92** (na začátek `add_building()`, před if) — přidat volání opravy normál:
```diff
...
**Řádek 104** — změnit merge střechy v `_add_pitched_building()`:
```diff
...
**Za řádek 169** (v `get_all_groups()`, za základním dict) — přidat klíč:
```diff
+        if self.is_lod0 and not self.gabled_roofs.is_empty():
+            groups['gabled_roofs_lod0'] = self.gabled_roofs
```

### Oprava obrácených normál valbových (hipped) střech

**Za `self.gabled_roofs = MeshData()`** v `__init__()` — přidat:
```diff
+        self.hipped_roofs = MeshData()
```

**V `add_building()`** — změnit volání `_fix_normals_outward` pro střechu:
```diff
-        _fix_normals_outward(result.walls, outer_ring, holes)
-        _fix_normals_outward(result.roof, outer_ring, holes)
+        _fix_normals_outward(result.walls, outer_ring, holes)
...
+            _fix_normals_outward(result.roof, outer_ring, holes)
```
*Důvod: valbové střechy mají `double_sided_roof=True` (záměrně horní i spodní plochy).
`_fix_normals_outward` by spodní plochy otočil nahoru a zničil double-sided efekt.*

**V `_add_pitched_building()`** — přidat větev pro hipped:
```diff
         if self.is_lod0 and result.actual_roof_type == RoofType.GABLED:
             self.gabled_roofs.merge(result.roof)
-        else:
...
**V `get_all_groups()`** — přidat klíč `hipped_roofs` (za blokem gabled_roofs):
```diff
...
**Řádky 80–83** — nahradit volání `_recalc_normals_outside` podmíněným:
```diff
-    _recalc_normals_outside(mesh)
+    # _recalc_normals_outside(mesh)  -- přepisovalo inner ring normály
+    if recalc_normals:
+        _recalc_normals_outside(mesh)
```
*Poznámka: `recalc_normals` je nový parametr (viz níže) — pro běžné objekty je `False`,
pro valbové střechy ho `import_grouped_meshes_to_blender` nastaví na `True`.*

**Signatura `meshdata_to_blender()`** — přidat parametr:
```diff
...
**Za řádek 453** (za funkcí `cleanup_buildings_collection`) — vložit dvě nové funkce:
```python
...
**Za řádek 492** (před hlavní smyčkou `for` v `import_grouped_meshes_to_blender()`) — přidat:
```diff
...
**Za řádek 495** (první řádek uvnitř smyčky `for group_name`) — přidat skip pro oba typy:
```diff
...
**Za řádek 510** (za `objects.append(obj)`) — zachytit houses_obj:
```diff
+        if group_name == 'houses':
+            houses_obj = obj
```

**Za koncem smyčky `for`** — přidat zdvojení sedlových a sloučení valbových:
```python
...
**Řádek 771** — přidat `double_sided_roof=False` do `GabledRoofConfig`:
```diff
...
**Za řádek 186** (za funkcí `_apply_terrain_orthophoto_uvs` nebo před ní) — vložit novou funkci:
```python
...
**Řádek 460** — přidat `is_lod0=True` do `grouper_lod0`:
```diff
...
**Řádek 697** — obalit `lod0_groups` při OBJ exportu:
```diff
...
**Za řádek 21** (před `def resolve_condor_paths`) — vložit novou funkci:
```python
...
**Řádek 657** — obalit `result.grouped_lod0` při exportu LOD0:
```diff
