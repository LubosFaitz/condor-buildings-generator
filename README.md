...
A new “Fence” block in “Other objects” builds a fence wherever OSM has a linear feature (way) tagged

> ### `barrier=fence`

Only ways are read. A fence tagged on a node is not built — a node itself has nothing to build, and `barrier=fence` there represents a point feature rather than the course of a fence. The new module can be removed:
...
The post is a ready-made model, not geometry generated by the code. It uses
...
The same material as the powerlines and aerialways is used. The material comes from

> ### `pylones` / `Pylons.dds`

The post is mapped to the wooden part of the texture, while the wire uses the dark grey part at the top. No new texture is required.

A fence crossing a patch border is not created twice. When a fence leaves the square of a patch, one post is created just beyond the patch border. Its height is read from the height map of the neighbouring patch, and its position is written to

> ### `Working/Autogen/fences_border.txt`

using the neighbouring patch's own coordinates. When the neighbouring patch is later built, it finds this post, skips creating it, and simply extends the wire to it. This works on all four sides and does not depend on the order in which patches are built.

Fence data is downloaded only when needed.

> ### `barrier=fence`

is too common a tag to include in the main OSM query, so it is left out. The patch currently being built makes one small, separate query for fences. If the patch has no fences, it is marked so that it will not unnecessarily query for them again.
