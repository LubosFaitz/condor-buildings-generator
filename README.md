...
A new **Fence** block in **Other objects** builds a fence wherever OSM has a linear feature (way) carrying this tag:

> **`barrier=fence`**

Only ways are read. A fence tagged on a node is not built — a node itself has nothing to build, and the tag there represents a point feature rather than the course of a fence. The new module can be removed:

> **`blender/fences.py`**

A post is placed at every corner, with approximately 4 m between them. No gap is ever longer than the configured spacing. The number of gaps is always rounded up, and the remaining length is distributed evenly between the two end gaps. For example:

> 10 m → **3 + 4 + 3 m**
>
> 14 m → **3 + 4 + 4 + 3 m**

The calculation is performed for each individual segment, so two posts are never placed next to each other at a corner, and no short “stub” is left at the end. The add-on preferences include a **Posts** spacing slider from 2–8 m, with a default value of 4 m.

The post is a ready-made model, not geometry generated by the code. It uses this file:

> **`assets/pylons/fence_post.obj`**

An octagonal wooden post with a diameter of 0.16 m, extending 1.30 m above the terrain and 1 m below the terrain, so no hole is visible on slopes. To change its shape, simply edit or redraw this one OBJ file. The code uses whatever is contained in the file.
...
The same material as the powerlines and aerialways is used, so no new texture is required:

> material **`pylones`**
>
> texture **`Pylons.dds`**

The post is mapped to the wooden part of the texture, while the wire uses the dark grey part at the top.

A fence crossing a patch border is not created twice. When a fence leaves the square of a patch, one post is created just beyond the patch border. Its height is read from the height map of the neighbouring patch, and its position is written, using the neighbouring patch's own coordinates, to:

> **`Working/Autogen/fences_border.txt`**

When the neighbouring patch is later built, it finds this post, skips creating it, and simply extends the wire to it. This works on all four sides and does not depend on the order in which patches are built.

Fence data is downloaded only when needed. The fence tag is too common to include in the main OSM query, so it is left out. The patch currently being built makes one small, separate query for fences. If the patch has no fences, it is marked so that it will not unnecessarily query for them again.
