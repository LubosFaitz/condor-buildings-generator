...
[![Version](https://img.shields.io/badge/version-0.9.12-blue.svg)](https://github.com/yourusername/condor-buildings-generator)
...
- **Bridge pillars every 50 m instead of 30 m**, and each one is now **collision-checked before it is built**. The check covers the pillar's whole volume — from two metres under the terrain up to the deck soffit — plus a 4 m protective zone, against everything this plugin generated (houses, other bridges, pylons, turbines, solar panels, substations, transmitters, chimneys). If the spot is taken, the pillar **slides along the deck to the nearest free place** (up to ±20 m) instead of leaving a wide unsupported gap; it is dropped only when that whole stretch is blocked, e.g. where another bridge crosses. In Blender the check can only see what already exists, so importing bridges before the buildings raises a warning.
