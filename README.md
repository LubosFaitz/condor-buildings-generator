...
- **After Merge the fence becomes part of `pylones`.** It shares the material and the texture with the powerlines and the aerialways, so it belongs in the same object: when the patch already has `pylones`, the fences are joined straight into it, and when it has none the merged fence is named `pylones` anyway. File mode does the same while writing the OBJ, where `pylones` is always the last object.
