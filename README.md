...
- **A post is placed at every corner, with approximately 4 m between them.** No gap is ever longer than the configured spacing. The number of gaps is always rounded up, and the remaining length is distributed evenly between the two end gaps. For example a 10 m segment becomes 3 + 4 + 3 m and a 14 m segment becomes 3 + 4 + 4 + 3 m. The calculation is performed for each individual segment, so two posts are never placed next to each other at a corner, and no short “stub” is left at the end. The add-on preferences include a **Posts** spacing slider from 2–8 m, with a default value of 4 m.
- **The post is a ready-made model, not geometry generated by the code.** It uses `assets/pylons/fence_post.obj` — a triangular wooden post with a diameter of 0.16 m, extending 1.30 m above the terrain and 1 m below the terrain, so no hole is visible on slopes. To change its shape, simply edit or redraw this one OBJ file. The code uses whatever is contained in the file.
- **Two wires are straight, with no sag.** The upper wire is 0.10 m below the top of the post, and the lower wire is 0.60 m below the top of the post. The wires pass through the posts and are connected only where the fence actually bends — either horizontally or due to a change in height. A long, straight and level section therefore consists of one continuous wire. The posts and wires are smooth-shaded, and the smoothing information is written into the OBJ.
- **The same material as the powerlines and aerialways is used.** The material comes from `pylones` / `Pylons.dds`. The post is mapped to the wooden part of the texture, while the wire uses the dark grey part at the top. No new texture is required.
- **A fence crossing a patch border is not created twice.** When a fence leaves the square of a patch, one post is created just beyond the patch border. Its height is read from the height map of the neighbouring patch, and its position is written to `Working/Autogen/fences_border.txt` using the neighbouring patch's own coordinates. When the neighbouring patch is later built, it finds this post, skips creating it, and simply extends the wire to it. This works on all four sides and does not depend on the order in which patches are built.
- **Fence data is downloaded only when needed.** `barrier=fence` is too common a tag to include in the main OSM query, so it is left out. The patch currently being built makes one small, separate query for fences. If the patch has no fences, it is marked so that it will not unnecessarily query for them again.
- **After Merge the fence becomes part of the powerline object `pylones`.** It shares the material and the texture with the powerlines and the aerialways, so it belongs in the same object. When the patch already has that object, the fences are joined straight into it. When it has none, the merged fence is given that name anyway. File mode does the same while writing the OBJ, where the powerline object is always written last.
- **Fences are never built into LOD1.** A fence is a small detail that is only seen from close up. With **LOD1** alone selected, Import only reports `Barrier fences are not created in LOD1` and stops, without downloading or computing anything. With **LOD0** or **Both LODs** the fence goes into LOD0 only, and in file mode it is written into `o<patch>.obj` while `o<patch>_LOD1.obj` is skipped.

**New in v0.9.15 — tree rows, hedgerows and single trees:**
- **A new *Tree row* block at the top of *Other objects*** builds trees from what is actually mapped in OSM, reading both ways and nodes. These are the tags it uses:

  | tag | what it is |
  |---|---|
  | natural=tree_row | an alley, a row of trees |
  | barrier=hedge | a hedgerow |
  | natural=hedge | the older spelling of a hedgerow |
  | barrier=hedge_bank | a bank with a hedge on top |
  | fence_type=hedge | a fence mapped as a hedge |
  | tree_lined on a road or a waterway | the trees lining it — also the namespaced tree_lined:left, tree_lined:right, tree_lined:both, plus denotation=avenue and the deprecated alley=left/right/both |
  | natural=tree | a single tree |

  The values "no" and "separate" are deliberately skipped, so nothing is ever built twice. New removable module `blender/tree_rows.py`.
- **The buildings download is untouched** — nothing extra is fetched for anyone who does not build trees. Only when the trees are built does each patch get checked, and a missing tag set is pulled in by ONE small query and merged into the existing `map_<patch>.osm` through a temp file, with an invisible marker so it is asked once.
- **Four categories, four models, one texture.** `tree_rows_1_alley`, `tree_rows_2_hedge`, `tree_rows_3_roadside` and `tree_rows_4_solitary` each take their own model from `assets/3Dobjects`, all mapped onto the single `tree_rows.dds`. Models keep their real size, shrunk up to 25 % at random; a mapped `height` scales them. A missing model falls back to a crossed billboard.
- **Spacing lives in the add-on preferences**, so it is remembered for every .blend: **Trees** 10–30 m (default 20) and **Hedges** 5–10 m (default 8). Hedge bushes are widened to span the spacing so a hedgerow is a continuous strip, and moving a slider rebuilds what is already in the scene.
- **No tree in the water** — water is read from the alpha of `t<patch>.dds` as the bridges do it; a tree landing on water is pushed off the centreline or dropped. `width` on a road or river sets the offset (half of it plus 3 m of bank).
- **Merge** joins each patch's categories into one `tree_rows` object with the `condor_tree` material; **Batch** writes that single merged block straight into `o<patch>.obj`.
- **Optional Copernicus layer** (off by default): fills in trees from the European Small Woody Features raster where OSM has nothing, one cached image per patch, keeping narrow strips and dropping solid blocks. Europe only. New removable module `blender/tree_rows_copernicus.py`.

**New in v0.9.14 - no more houses where there is no house:**
- **The value of the `building` tag is now checked, not just its presence.** Anything carrying the word `building` used to be built, whatever stood after it, so a coastal staircase explicitly tagged `building=no` came out as a flat-roofed house shaped like the zig-zag of the steps. Nothing is built any more for `building=no` (the mapper states outright it is not a building), `ruins`, `roof` (a roof on posts with no walls - bus shelter, fuel station canopy), `bridge` and `construction`. A structure tagged `amenity=shelter` is skipped as well, even when it carries `building=yes`. Everything else - houses, apartments, halls, churches, garages - is built exactly as before, MSprint buildings included.

**New in v0.9.13 — no buildings on water, one mask per solar farm, bridges that follow the track:**
- **Buildings standing on water are dropped** — the checkbox is now **Exclude airports, solar farms and water**. The water is read from the **alpha channel of the patch texture** `t<patch>.dds`, the same source the bridges use, so it matches what you see in Condor. Only a footprint that is water at **every** one of its points goes; a single point on dry land keeps the building, so quaysides and riverbank houses stay. It helps most with MSprint, whose machine-derived footprints occasionally land in a river. Needs a DXT3 texture (a DXT1 scenery simply skips this part).
- **One solar power station = one satellite cut-out and one mask.** A big farm is often mapped in OSM twice over — the whole site, plus every block of panels inside it as its own area. Kencot Hill (UK patch 039034) came out as **308 "farms", 1185 files and 258 downloaded tiles for a single station**, which made the mask impossible to edit. Blocks lying inside a site outline are now dropped, outlines closer than 50 m are joined into one, and the tile ceiling was raised so a whole joined farm still downloads. That patch now yields **2 farms instead of 308**.
- **A bridge crossing the patch border is built only once.** It used to be built WHOLE by both neighbours — two identical decks on top of each other. A register in `Working/Autogen/bridges/built.json` records which patch built which bridge (only those actually reaching past the patch edge); the neighbour skips it, and a patch may always rebuild its own.
- **Merged bridge decks follow the real route.** Where several bridges run side by side (multi-track rail, dual carriageway) they are built as one deck — which used to be run dead straight from end to end, so a curving viaduct cut through the houses the track goes around. London patch 020029: a 2.6 km rail viaduct came out as a straight slab through the city; it now follows the track, **deviating 386 m from the straight line**. Bridges that are genuinely straight are unchanged.
- Pillar protective zone reduced from 4 m to **2 m**.

**New in v0.9.12 — keeping autogen out of airfields & smarter bridge pillars:**
- **Exclude airports and solar farms** — a new checkbox in the *OSM Data* box, right under *MSprint*, **off by default**. With it on, **no object is generated inside an airfield or a ground solar farm** — no buildings (not even MSprint ones), no wind turbines, no aerialways. A Condor scenery usually already has its own hand-made objects at an airfield, so the generated ones only had to be deleted again by hand; in a solar farm the panels come from the *Solar farms* tool. **Power lines are never excluded** — they have to cross the area. The airfield is taken from the OSM outline (area or multipolygon); a small field with no outline gets a corridor 200 m to each side of its runway and 200 m past both ends; with neither, a 1.2 km square around the node. Solar farms use exactly the same tags the solar tool uses, and rooftop arrays are ignored. The download now also fetches aerodrome relations and solar-farm outlines, so **`map_*.osm` files downloaded earlier have to be deleted and fetched again** for the exclusion to see them.
- **Bridge pillars every 50 m instead of 30 m**, and each one is now **collision-checked before it is built**. The check covers the pillar's whole volume — from two metres under the terrain up to the deck soffit — plus a 2 m protective zone, against everything this plugin generated (houses, other bridges, pylons, turbines, solar panels, substations, transmitters, chimneys). If the spot is taken, the pillar **slides along the deck to the nearest free place** (up to ±20 m) instead of leaving a wide unsupported gap; it is dropped only when that whole stretch is blocked, e.g. where another bridge crosses. In Blender the check can only see what already exists, so importing bridges before the buildings raises a warning.

**New in v0.9.11 — Laplacian Smooth for the terrain (tessellation simulation):**
- **Optional terrain smoothing, off by default** — a new checkbox in *Edit > Preferences > Add-ons > Condor Buildings Generator*: **Enable Laplacian Smooth Modifier for Terrain (Tessellation Simulation)**. Condor tessellates the terrain when it loads a landscape, so the surface you actually fly over is smoother than the raw `h<patch>.obj`. With the checkbox on, the terrain is smoothed the same way Blender's **Smooth (Laplacian)** modifier does it, and **all autogen sits on that smoothed surface** — buildings, power lines, aerialways, bridges, wind turbines, solar farms, substations, chimneys and transmitters alike, in Blender as well as in file/batch mode.
- **The whole modifier is exposed** — Repeat (default 2), Lambda Factor (default 4.0), Lambda Border (default 0, keeps the patch edges in place so no gap appears towards the neighbour), Axis X / Y / Z (only **Z** on by default, so heights change and nothing shifts sideways), Preserve Volume and Normalized. Being add-on preferences, the settings are remembered across scenes and Blender restarts.
- **The terrain files are never touched** — only the computed heights are stored, one value per line in `Working/Heightmaps/smooth/h<patch>.txt` (about 290 kB per patch, six times less than the terrain itself); the mesh, faces and X/Y always come from the original. A hand-edited terrain in `modified` still takes precedence and is smoothed instead. The result is cached and recomputed by itself when a setting changes or the source terrain is swapped or re-saved.
- **Export Terrain no longer triangulates** — the standard 30 m terrain is written back to `modified` with its **quads** (only the detailed 22.5 m terrain, `tr3f`, is saved triangulated, because that one really is).

**New in v0.9.10 — finding solar farms & smoother solar workflow:**
- **Scan solar farms** — a new button in the *Solar* row that lists which patches of the whole scenery contain a ground solar farm (with the farm names), written to `Working/Autogen/solar_farms.txt`. It's a **finder, not a builder** — so you know where the farms are and can build just the patches you want instead of trying them all. It runs in its **own window outside Blender** (Blender stays usable) and is offered only while that list doesn't exist yet; when the scan finishes the button disappears on its own. The area is cut into query bands along whichever side gives the **fewest bands**, to stay under the OpenStreetMap request limit.
- **Solar flip survives a re-import** — when you redraw one farm's mask and press Import again, the other farms keep the tilt you gave them (remembered per farm, also on disk so it survives closing Blender).
- **Mask editing hardened** — the *Mask on terrain* plane now floats high above the terrain so it stays visible on a slope, can only be moved and turned (tilt/scale locked, since the imagery is top-down), and **Save mask** rewrites only a mask you actually moved — a finished mask can't be spoiled. **Merge** also clears the mask planes away by itself.

**New in v0.9.9 — substations (gantries) & curved bridge decks:**
- **Substation gantries** — where a power line enters a `power=substation`, a **gantry** (`pylon_substation`) is now built at the fence and the cables connect to it, so lines no longer end in mid-air. The pylon that gets one is the last one **outside** the outline whose **neighbouring node on the same OSM way lies inside** it (cable count and fence distance are irrelevant; small pylons skipped, one gantry per pylon).
- **Gantry placement** — a **perpendicular** from the pylon centre must **actually hit an outline wall**; the nearest such wall wins (even if far behind a corner). The gantry sits **16 m inside** that wall, **aligned with it**, facing the pylon; nearest fence point only as a fallback.
- **No overlaps, no crossed cables** — gantries only move when they **genuinely intersect** (exact rotated-rectangle / SAT test, 12.2 × 5.5 m). The pair then slides along the fence in 0.2 m steps, never leaving the fence (centre + 4 corners checked); a blocked gantry passes the shift to the other one, and the order around the whole (CCW) perimeter is preserved so cables can't cross. A pylon very close to the fence increases the offset to keep ≥ 25 m between pylon and gantry.
- **Cable attachment** — the last span uses the **gantry's** attachment points (3 points 4.61 m apart, 14.07 m above terrain) instead of the incoming line's, paired by side; the **earth wire from the pylon top is not connected** to the gantry. Ordinary spans unchanged. Changes are confined to `generators/powerlines.py`; the `pylon_substation.obj` model itself is untouched.
- **Bridges keep their curve over water** — `_span_water` now follows the road's real OSM path: it finds where the route enters/leaves the water from the orthophoto alpha, **keeps the mapped points across the water** (so a bent bridge stays bent) and only runs the two **ends** straight onto the dry banks (`BANK_MARGIN`). Long dry approaches are trimmed; a route crossing no water is left as mapped.
...
- Fences (`barrier=fence`) — wooden posts every 4 m with two sagless wires, sharing the `pylones` material / `Pylons.dds`; a fence crossing a patch border is built only once
...
| 0.9.9 | Jul 16, 2026 | **Substation gantries** (`generators/powerlines.py` only; `pylon_substation.obj` untouched): a gantry is built where a line enters a `power=substation` — the pylon chosen is the last one **outside** the outline whose **neighbouring node on the same OSM way is inside** (verified on patch 024029: 12 pylons = the real number of connections). Placement by a **perpendicular that must actually hit an outline wall** (nearest such wall, 16 m inside, aligned with it; nearest fence point as fallback). Overlaps resolved **only on a genuine intersection** (SAT / OBB 12.2 × 5.5 m): the pair slides along the fence in 0.2 m steps, stays inside (centre + 4 corners), a blocked gantry hands the shift to the other, CCW perimeter order preserved → **cables never cross**; ≥ 25 m kept between a fence-hugging pylon and its gantry. Last span uses the **gantry's** attachment points (3 × 4.61 m apart at 14.07 m), paired by side, **earth wire not connected** (`_span_cables_mixed`). **Bridges**: `_span_water` follows the road's real OSM path — mapped points across the water are kept (curved decks stay curved), only the ends run straight onto the dry banks; dry approaches trimmed |
| 0.9.10 | Jul 17, 2026 | **Scan solar farms** — a standalone helper (`blender/solar_scan.py`) run in its own process from a new *Solar* button that lists which patches of the scenery hold a ground farm (`Working/Autogen/solar_farms.txt`, farm names included), so only wanted patches need building; the button shows only while the list is missing and the scan cuts query bands along the shorter axis to stay under the Overpass limit. **Solar flip** now survives a re-import (kept per farm, cached in `solar_flip.json`). **Mask editing**: the *Mask on terrain* plane floats ≈55 m up (visible on slopes) with tilt/scale locked, **Save mask** rewrites only a plane that was actually moved, and **Merge** clears the mask planes away |
| 0.9.11 | Jul 28, 2026 | **Laplacian Smooth for the terrain (tessellation simulation)** — optional, **off by default**, switched on in *Preferences > Add-ons*: the terrain is smoothed exactly as Blender's Smooth (Laplacian) modifier does it (`blender/terrain_smooth.py`) and **all autogen sits on that surface** — buildings, power lines, aerialways, bridges, turbines, solar, substations, chimneys, transmitters, in Blender and in file/batch mode alike. The full modifier is exposed (Repeat 2, Lambda Factor 4.0, Lambda Border 0, Axis X/Y/**Z**, Preserve Volume, Normalized) and remembered across scenes and restarts. Only the **heights** are stored, one per line in `Working/Heightmaps/smooth/h<patch>.txt` (~290 kB, 6× less than the terrain); mesh, faces and X/Y always come from the original, which is never written to. `modified` keeps its precedence and is smoothed instead; the cache recomputes by itself when a setting changes or the source is swapped/re-saved. Also: **Export Terrain no longer triangulates** the standard 30 m terrain (quads are kept; only `tr3f` is saved triangulated), and the sidebar stays on the **Condor** tab after generating |
| 0.9.12 | Aug 11, 2026 | **Exclude airports and solar farms** — a new *OSM Data* checkbox (**off by default**): inside an airfield or a ground solar farm **nothing is generated** — no buildings (MSprint ones included), no wind turbines, no aerialways — while **power lines always pass through**. Airfields come from the OSM `aeroway=aerodrome` outline (area **or multipolygon**, newly downloaded), else a **runway corridor** (200 m to each side, 200 m past both ends — covers fields like LKRY that have no outline), else a 1.2 km square around the node; solar farms use exactly the tags `blender/solar.py` uses, rooftop arrays ignored. Older `map_*.osm` must be re-downloaded to contain the new outlines. **Bridge pillars**: spacing **30 m → 50 m**, and every pillar is **collision-checked before it is built** — its whole volume (2 m under the terrain up to the deck soffit) plus a 2 m protective zone against everything the plugin generated (houses, other bridges, pylons, turbines, solar panels, substations, transmitters, chimneys); the terrain and the pillar's own deck don't count. A blocked pillar **slides along the deck to the nearest free spot** (±40 % of the spacing) instead of leaving a gap, and is dropped only when the whole stretch is taken. Obstacles are read from the patch OBJ in file mode (bridges are appended last) or from the patch collection in Blender, where importing bridges before the buildings raises a warning |
| 0.9.13 | Aug 12, 2026 | **Buildings on water dropped** (checkbox renamed *Exclude airports, solar farms and water*): the water comes from the **alpha channel of `t<patch>.dds`** (DXT3 decoded in `main.py`, independent of the removable `bridges.py`), and only a footprint that is water at EVERY point is removed, so quaysides survive; helps most against stray MSprint footprints. **Solar: one station = one cut-out + one mask** — panel blocks lying inside a `power=plant` outline are dropped and outlines within `FARM_MERGE_GAP` (50 m) are joined by convex hull, turning Kencot Hill's 308 "farms"/1185 files into 2; `ESRI_MAX_TILES` 80 → 400 so a whole joined farm still downloads. **Bridges: no double build across a patch border** — `bridges/built.json` records patch ownership of bridges that stick out past the edge, first patch wins, a patch may always rebuild its own. **Bridges: merged decks keep the real path** — `_merge_crossing` used to replace a fused deck with a straight two-point line, straightening a curving viaduct through the houses (London 020029: a 2.6 km rail viaduct, now deviating 386 m from the straight line as it should); genuinely straight decks unchanged, five other patches byte-identical. Pillar clearance 4 m → 2 m |
...
| 0.9.19 | Sep 2, 2026 | **A leaner `.c3d`** — the writer stored more points than the format needs. A point in a `.c3d` carries its position plus the direction it faces and how the texture lies on it, so two points agreeing in all of that need storing only once; the comparison is meant to run on **rounded** values (position to seven digits, direction and texture to five), but the plugin compared them exactly, so a difference in the sixth decimal place made two points count as different and both were written — almost five thousand surplus points on a single tile. Points are now rounded before the comparison (seven digits for the position, five for the direction and the texture), merged **across the whole model** instead of only within one group, a face with no normal given is written as zero, and the texture-flip convention is corrected (1 − v instead of −v). The result is **byte-for-byte identical** to the file the landscape editor produces, verified by checksum on tile o036019; it holds for the **Export C3D** button and the **Save as C3D** checkbox alike |
