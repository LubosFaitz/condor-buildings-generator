...
[![Version](https://img.shields.io/badge/version-0.9.16-blue.svg)](https://github.com/yourusername/condor-buildings-generator)
...
**New in v0.9.16 — fences:**
- **A new *Fence* block in *Other objects*** builds a fence wherever OSM has a line tagged `barrier=fence`. Only ways are read — a fence on a node has nothing to build, and `area=yes` is the outline of a plot, not a run of fence. New removable module `blender/fences.py`.
- **A post at every corner, 4 m apart in between.** No gap is ever longer than the spacing: the number of gaps is rounded UP and the remainder is shared equally between the two end gaps, so a 10 m stretch comes out as 3 + 4 + 3 and a 14 m one as 3 + 4 + 4 + 3. Worked out per segment, so a corner never gets two posts and no stub is left at the end. Spacing slider **Posts** (2–8 m, default 4) in the add-on preferences.
- **The post is a model, not generated** — `assets/pylons/fence_post.obj`, an octagonal wooden post 0.16 m across, 1.30 m above the terrain and 1 m sunk below it so no hole shows on a slope. Redraw that one file to change its shape; the code reads whatever is in it.
- **Two wires that do not sag** — the top one 0.10 m below the head of the post, the lower one 0.60 m below that. They run THROUGH the posts and are jointed **only where the fence really bends**, in plan or in height, so a straight flat run is one unbroken wire. Post and wires are smooth-shaded, and the smoothing is written into the OBJ.
- **The same material as the powerlines and aerialways** — `pylones` on `Pylons.dds`. The post is mapped onto the wooden strip in that texture, the wire takes the dark grey block at the top. No new texture needed.
- **A fence crossing the patch border is not built twice.** Where a fence leaves the square, one post is put up just beyond the edge — its height read from the **neighbouring patch's** height map — and written into `Working/Autogen/fences_border.txt` in the neighbour's own coordinates. The neighbour then finds it standing, skips it and simply strings the wire to it. Works through all four sides at once and in any build order.
- **Fetched on demand** — `barrier=fence` is far too common a tag for the main query, so it is left out of it: only a patch actually being built asks for its fences, with one small query of its own, and a patch that has none is marked so it is not asked again.
...
| 0.9.16 | Aug 31, 2026 | **Fences** — a new removable module `blender/fences.py` builds a fence from every OSM line tagged `barrier=fence` (ways only; `area=yes` is an outline, not a fence). A post stands at **every corner** and the stretches between them are filled at **4 m** with no gap ever longer than that: the number of gaps is rounded up and the remainder shared equally between the two end gaps, so 10 m comes out as 3 + 4 + 3 and 14 m as 3 + 4 + 4 + 3, worked out per segment so a corner is never doubled and no stub is left. The post is the model `assets/pylons/fence_post.obj` (octagonal, 0.16 m across, 1.30 m up and 1 m sunk, so no hole shows on a slope) — redraw that one file to change its shape. **Two sagless wires**, the top 0.10 m under the head of the post and the lower 0.60 m below it, run THROUGH the posts and are jointed **only where the fence really bends**, in plan or in height, so a straight flat run is one unbroken wire; post and wires are smooth-shaded and the normals are written into the OBJ. The **`pylones` material / `Pylons.dds`** is shared with the powerlines and aerialways — the post uses the wooden strip, the wire the dark grey block at the top — so no new texture is needed. **One fence = one object** `fence`, **Merge** joins a patch. A fence crossing a patch border gets **one post just beyond the edge, at the NEIGHBOUR's terrain height**, recorded in `Working/Autogen/fences_border.txt` in the neighbour's own coordinates, so the neighbour finds it standing, skips it and only strings the wire to it — all four sides at once, any build order, nothing built twice. `barrier=fence` stays **out of the main query** and is fetched per patch on demand, a patch with none marked so it is not asked again. Spacing slider **Posts** (2–8 m, default 4) in the add-on preferences |
