...
- **The post is a ready-made model, not geometry generated by the code.** It uses `assets/pylons/fence_post.obj` — an octagonal wooden post with a diameter of 0.16 m, extending 1.30 m above the terrain and 1 m below the terrain, so no hole is visible on slopes. To change its shape, simply edit or redraw this one OBJ file. The code uses whatever is contained in the file.
