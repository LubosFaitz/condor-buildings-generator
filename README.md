...
[![Version](https://img.shields.io/badge/version-0.9.15-blue.svg)](https://github.com/yourusername/condor-buildings-generator)
...
- **A new *Tree row* block at the top of *Other objects*** builds trees from what is actually mapped in OSM — on ways and on nodes alike: `natural=tree_row`, `barrier=hedge`, `natural=hedge`, `barrier=hedge_bank`, `fence_type=hedge`, roads and waterways carrying `tree_lined` (also `tree_lined:left/:right/:both`, `denotation=avenue`, the old `alley=*`), and `natural=tree`. `no` and `separate` are deliberately skipped. New removable module `blender/tree_rows.py`.
